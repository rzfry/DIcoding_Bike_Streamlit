...
with st.sidebar:
    st.image("dashboard/bike wotbg.png") #logo image
...
st.header('Streamlit Dashboard Bike Rental')
...
with col1:
    ...
    st.metric("Total Rentals (units)", value=total_rentals)
 
with col2:
    total_registered = daily_rentals_df.total_registered.sum()
    st.metric("Total Registered Customer (account)", value=total_registered)

with col3:
    total_casual = daily_rentals_df.total_casual.sum()
    st.metric("Total Casual Customer (account)", value=total_casual)
...
weather_colors = {
    'Clear':'gold', 
    'Cloudy':'lightblue', 
    'Light Snow/Rain':'black', 
    'Heavy Rain/Ice Pallets':'gray'
}
...
deskripsi = """
Selamat datang di layanan **Rental Sepeda Gunung Kelud**, penyedia sepeda gunung terbaik di kawasan wisata Gunung Kelud!  
Kami menawarkan pengalaman bersepeda yang tak terlupakan di jalur-jalur menantang dan indah, cocok bagi para pencinta alam, petualang, dan penggemar olahraga sepeda.

### Apa yang Kami Tawarkan?
- **Sepeda Gunung Berkualitas Tinggi**: Sepeda modern dengan fitur lengkap untuk menghadapi medan pegunungan.
- **Paket Rental Fleksibel**: Tersedia pilihan sewa per jam, harian, hingga paket wisata bersepeda.
- **Pemandu Profesional (Opsional)**: Pemandu berpengalaman untuk membantu Anda menjelajahi jalur terbaik di Gunung Kelud.
- **Peralatan Lengkap**: Termasuk helm, perlengkapan keselamatan, dan aksesori tambahan.

### Keunggulan Kami:
- Dekat dengan pintu masuk kawasan wisata Gunung Kelud.
- Sepeda selalu dirawat dan diperiksa untuk memastikan keamanan dan kenyamanan.
- Harga sewa terjangkau dengan layanan pelanggan ramah.

### Jelajahi Keindahan Gunung Kelud
Dengan sepeda gunung kami, Anda dapat menikmati keindahan alam Gunung Kelud dari dekat: jalur hutan, pemandangan kawah, hingga udara segar khas pegunungan.  
Rasakan sensasi berpetualang sambil menjaga kebugaran tubuh!

**Hubungi kami sekarang dinomer berikut +6289515949104** dan rencanakan perjalanan bersepeda Anda di Gunung Kelud!
"""

st.markdown(deskripsi)
st.caption('Copyright © Ayam Geprek Nyam-Nyam 2024')